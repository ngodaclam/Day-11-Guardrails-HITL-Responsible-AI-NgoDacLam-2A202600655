...
def build_notebook():
        ...
        "cells": [
            {
                ...
                "source": [
                    ...
                    "\n",
                    "This notebook implements a production-grade defense-in-depth safety pipeline containing 6 independent safety layers, monitoring, and audit logging."
                ]
            },
            {
                "cell_type": "markdown",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "            return types.Content(role='model', parts=[types.Part.from_text(text=f'Request blocked: Rate limit exceeded. Please wait {wait_time} seconds before trying again.')])\n",
                    "        window.append(now)\n",
                    "        return None\n",
                    "\n",
                    ...
                    "            return types.Content(role='model', parts=[types.Part.from_text(text='Request blocked: Prompt injection detected.')])\n",
                    "        if topic_filter(text):\n",
                    "            self.blocked_count += 1\n",
                    "            return types.Content(role='model', parts=[types.Part.from_text(text='Request blocked: Off-topic banking query.')])\n",
                    "        return None\n",
                    "\n",
                    ...
                    "        self.judge_agent = llm_agent.LlmAgent(model='gemini-2.5-flash', name='judge_assistant', instruction=JUDGE_INSTRUCTION)\n",
                    ...
                    "                self.judge_fails += 1\n",
                    "                llm_response.content = types.Content(role='model', parts=[types.Part.from_text(text='Request blocked: Response safety check failed.')])\n",
                    "        except Exception as e:\n",
                    "            print(f'LLM Judge query failed: {e}')\n",
                    "        return llm_response\n",
                    "\n",
                    ...
                    "        self.logs.append({'timestamp': datetime.now().isoformat(), 'user_id': user_id, 'type': 'input', 'content': text})\n",
                    "        return None\n",
                    "    async def after_model_callback(self, *, callback_context, llm_response):\n",
                    ...
                    "        self.logs.append({'timestamp': datetime.now().isoformat(), 'user_id': user_id, 'type': 'output', 'content': text, 'latency_seconds': latency})\n",
                    "        return llm_response\n",
                    ...
                    "            return types.Content(role='model', parts=[types.Part.from_text(text='Request blocked: Daily API budget exceeded.')])\n",
                    "        text = self._extract_text(user_message)\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "                print(f'  [Output Guardrail] Redacted: {p.redacted_count}')\n",
                    "            elif p.name == 'llm_judge':\n",
                    "                print(f'  [LLM-as-Judge] Fails: {p.judge_fails}')\n",
                    "            elif p.name == 'cost_guard':\n",
                    "                print(f'  [Cost Guard] Blocked hits: {p.blocked_cost_count}')\n",
                    "                print(f'  [Cost Guard] Budget spent: ${p.total_cost_usd:.5f}')\n",
                    "        print('=' * 40 + '\\n')\n"
                ]
            },
            {
            ...
            },
            {
                ...
                "source": [
                    ...
                    "cost_guard = CostGuardPlugin(daily_budget_usd=0.05)\n",
                    "\n",
                    "production_plugins = [rate_limiter, input_guard, output_guard, llm_judge, audit_log, cost_guard]\n",
                    "\n",
                    ...
                    ")\n",
                    "\n",
                    "runner = runners.InMemoryRunner(agent=agent, app_name='protected_test_run', plugins=production_plugins)\n",
                    "print('Safety Pipeline Initialized.')\n"
                ]
            },
            {
                "cell_type": "code",
                ...
                "outputs": [],
                "source": [
                    "async def run_test_suite(name, queries):\n",
                    ...
                    "]\n",
                    "await run_test_suite('SAFE QUERIES', safe_queries)\n"
                ]
            },
            {
                "cell_type": "code",
                ...
                "outputs": [],
                "source": [
                    "# 2. Test Attacks (Should be BLOCKED)\n",
                    "attack_queries = [\n",
                    ...
                    "]\n",
                    "await run_test_suite('ATTACK QUERIES', attack_queries)\n"
                ]
            },
            {
                "cell_type": "code",
                ...
                "source": [
                    ...
                    "    'What is 2+2?',\n",
                    "]\n",
                    "await run_test_suite('EDGE CASES', edge_cases)\n"
                ]
            },
            {
                "cell_type": "code",
                ...
                "outputs": [],
                "source": [
                    "# 4. Test Rate Limiter (send 15 rapid requests)\n",
                    "print('=== Testing Rate Limiter (15 rapid requests) ===')\n",
                    "for i in range(15):\n",
                    "    response, _ = await chat_with_agent(agent, runner, 'Check my balance')\n",
                    "    print(f'Req #{i+1}: {response[:80]}')\n"
                ]
            },
            {
                "cell_type": "code",
                ...
                "outputs": [],
                "source": [
                    "# 5. Check Metrics & Export Logs\n",
                    "monitor = MonitoringAlert(production_plugins)\n",
                    "monitor.check_metrics()\n",
                    "\n",
                    "audit_log.export_json()\n",
                    "print('Audit log successfully exported to audit_log.json')\n"
                ]
            }
        ],
        "metadata": {
    ...
    notebook_dir = r"c:\Users\LocND\Desktop\api\Day-11-Guardrails-HITL-Responsible-AI-NgoDacLam-2A202600655\notebooks"
    os.makedirs(notebook_dir, exist_ok=True)
    notebook_path = os.path.join(notebook_dir, "assignment11_defense_pipeline.ipynb")
